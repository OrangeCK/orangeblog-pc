    ...
    <div>
        <div class="min_tools">
            <div class="tools" ref="tools" :class="{'is_fixed' : isFixed}">
                ...
                <Input  placeholder="请输入字典名称..." v-model="searchForm.dicValue" clearable class="inp"/>
                <Button type="warning" custom-icon="iconfont icon-sousuo" @click="searchDictionary(1)">Search</Button>
                <Button type="warning" custom-icon="iconfont icon-jiajianzujianjiahao" @click="addDictionary">Add</Button>
            </div>
        </div>
        ...
        <div class="content">
            ...
            <div style="margin: 10px 10px 0 10px;overflow: hidden">
                <div style="float: right;">
                    <Page :total="dTable.page.total" :page-size="dTable.page.pageSize" :current="dTable.page.pageIndex" @on-change="searchDictionary"
                        @on-page-size-change="handlePageSize" show-sizer show-total show-elevator/>
                </div>
            </div>
            <Modal :mask-closable="addModal.closable" :closable="addModal.closable" v-model="addModal.show" :title="addModal.title" @on-cancel="cancelDictionary('addModal.form')">
                ...
                <Form :model="addModal.form" ref="addModal.form" :rules="addModal.ruleValidate" :label-width="90" >
                    <FormItem label="字典编码" prop="dicCode" >
                        <Input v-model="addModal.form.dicCode" placeholder="请输入字典编码"/>
                    </FormItem>
                    <FormItem label="字典名称" prop="dicValue" >
                        <Input v-model="addModal.form.dicValue" placeholder="请输入字典名称"/>
                    </FormItem>
...
export default {
                            ...
                            render:(h, params) => {
                                return h('div',[
                                    ...
                                    h('Button', {
                                        ...
                                        on: {
                                            click: () => {
                                                this.deleteDictionary(params.row.id);
                                            }
                                        }
                                    }, 'Del')   
                                ])
        ...
        methods: {
            ...
            searchDictionary(page){
                ...
                this.service({
                    url: this.API.dictionaryPageList,
                    data: {
                        'dicCode':this.searchForm.dicCode,
                        'dicValue':this.searchForm.dicValue,
                        'pageIndex':page,
            ...
            submitDictionary(name){
                 this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        let url = this.API.saveDictionary;
                        this.service({
                            url: url,
                            data: {
            ...
            deleteDictionary(id){
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        this.handleSpinShow();
                        this.service({
                            url: this.API.deleteDictionary + "?id="+id,
                            method: "post"
                        }).then(response => {
                            this.handleSpinHide();
